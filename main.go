	...
	http.HandleFunc("/", func(w http.ResponseWriter, r *http.Request) {
		if r.Method == "GET" {
			...
			if !isCodeSuccessful(city.statusCode) || !isCodeSuccessful(weather1.statusCode) || !isCodeSuccessful(weather2.statusCode) {
				generateErrHTML(w, city, weather1, weather2)
...
func generateErrHTML(w http.ResponseWriter, cityAPIResponse CityAPIResponse, weatherAPIResponse1, weatherAPIReponse2 WeatherAPIResponse) {
	responseHTML := fmt.Sprintf(`
                <html>
                    <body>
            ...
            `, cityAPIResponse.cityName, cityAPIResponse.statusCode, cityAPIResponse.status, weatherAPIResponse1.apiName, weatherAPIResponse1.statusCode, weatherAPIResponse1.status, weatherAPIReponse2.apiName, weatherAPIReponse2.statusCode, weatherAPIReponse2.status)
